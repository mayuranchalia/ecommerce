component  hint="This is persistance implementation to persist/retrieve brand information from database"
{
	function saveBrand(brandName,brandDescription) returntype="ecommerce.model.brand" access="public"     {
		
		queryexecute("insert into brand_table (brand_name,brand_description) values(?,?)",
			[brandName,brandDescription],{result="result"});
			
			brand = createObject("component", "ecommerce.model.brand");
	...
	function getBrands() returntype="ecommerce.model.brand[]" access="public"      {
	...
	function getBrandDetailsById(brandIdArg) returntype="ecommerce.model.brand" access="public" {
		...
		brand = createObject("component", "model.brand");
